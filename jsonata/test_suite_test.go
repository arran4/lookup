...
func runTxtarGroup(t *testing.T, filename string, groupName string) {
	...
	for _, c := range cases {
		t.Run(c.Name, func(t *testing.T) {
			...
			out, actualUndefined = materializeHarnessValue(res)
			match := false
			if sc.Undefined {
...
func evaluateHarnessOutcome(testID string, execErr error, scCode string, expectPass bool, isUnsupported bool, unsupportedReason string, isUndefined bool, setupErr error) harnessOutcome {
	...
	if scCode != "" {
		...
		return harnessOutcome{Message: "pass-execution-error"}
	}
	if execErr != nil {
		if isUndefined {
			var invalidor *lookup.Invalidor
			if errors.As(execErr, &invalidor) && IsUndefinedError(invalidor) {
...
func materializeHarnessValue(v interface{}) (value interface{}, undefined bool) {
	if inv, ok := v.(*lookup.Invalidor); ok && IsUndefinedError(inv) {
		return nil, true
	}
...
func jsonataValuesEqual(expected, actual interface{}) bool {
	...
	if ev.IsValid() && av.IsValid() && (ev.Kind() == reflect.Slice || ev.Kind() == reflect.Array) && (av.Kind() == reflect.Slice || av.Kind() == reflect.Array) {
		if ev.Len() != av.Len() {
			return false
		}
		for i := 0; i < ev.Len(); i++ {
