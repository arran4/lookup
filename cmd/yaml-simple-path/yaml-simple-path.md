...
yaml-simple-path - query YAML files using lookup paths

# SYNOPSIS

`yaml-simple-path [options] PATH [PATH ...]`
...
```
$ yaml-simple-path -grep '^prod' .metadata.name doc.yaml
prod-service
```

```
$ yaml-simple-path -count .metadata.name doc.yaml
1
```
