...
json-simple-path is a small command line tool built on top of the lookup library. It reads one or more JSON documents and extracts values using lookup's `SimplePath` syntax. The interface should feel familiar to Unix users with options inspired by `cut`, `sed`, `grep` and modern tools such as `jq`.

```
Usage: json-simple-path [options] PATH [PATH ...]
...
$ json-simple-path -raw .spec.replicas < doc.json
3

# Only show values matching a pattern
$ json-simple-path -grep '^prod' -raw -f doc.json .metadata.name
prod-service

# Count how many documents have a metadata.name field
$ json-simple-path -count -f doc.json .metadata.name
1
```
